...
function setupShaders() {
  ...
  var fShaderCode = `
        ...
        void main(void) {
            gl_FragColor = vec4(fragColor, 1.0); // fragment color
        }
    `;
...
function main() {
  // Show part 5 image on tapping space bar
  window.addEventListener("keydown", function (event) {
    // Check if the pressed key is the space bar
    if (event.code === "Space") {
      setupWebGL(); // set up the webGL environment
      loadTriangles("https://ncsucgclass.github.io/prog1/triangles2.json"); // load in the triangles from tri file
      setupShaders(); // setup the webGL shaders
      renderTriangles(); // draw the triangles using webGL
    }
  });

  // Load the part 4 image
  setupWebGL(); // set up the webGL environment
